...
public class SoldierAvatar
{
    ...
    public void Initialize(GameObject go)
    {
		...
		//VT
		rightWrist = UnityUtils.FindTransform(go, "R_Hand");
		...
		//VT
		initialRotations[(int)NiteWrapper.SkeletonJoint.RIGHT_WRIST] = rightWrist.rotation;
    ...
    public void UpdateAvatar(uint userId)
    {
        ...
        TransformBone(userId, NiteWrapper.SkeletonJoint.TORSO_CENTER, spine, true);
        TransformBone(userId, NiteWrapper.SkeletonJoint.RIGHT_SHOULDER, rightArm, false);
        TransformBone(userId, NiteWrapper.SkeletonJoint.LEFT_SHOULDER, leftArm, false);
        TransformBone(userId, NiteWrapper.SkeletonJoint.RIGHT_ELBOW, rightElbow, false);
		
        TransformBone(userId, NiteWrapper.SkeletonJoint.LEFT_ELBOW, leftElbow, false);
        TransformBone(userId, NiteWrapper.SkeletonJoint.RIGHT_HIP, rightHip, false);
        TransformBone(userId, NiteWrapper.SkeletonJoint.LEFT_HIP, leftHip, false);
        TransformBone(userId, NiteWrapper.SkeletonJoint.RIGHT_KNEE, rightKnee, false);
        TransformBone(userId, NiteWrapper.SkeletonJoint.LEFT_KNEE, leftKnee, false);
    }
    ...
    public void RotateToInitialPosition()
    {
		...
		//VT
		rightWrist.rotation = initialRotations[(int)NiteWrapper.SkeletonJoint.RIGHT_WRIST];
    ...
    void TransformBone(uint userId, NiteWrapper.SkeletonJoint joint, Transform dest, bool move)
    {
		...
		if (move)
		{
			dest.position = new Vector3(trans.pos.x/100 , trans.pos.y/200  + 2.0f, -trans.pos.z/200 + 10);
		}
    }
}
...
public class Nite : MonoBehaviour
{
    ...
    void Start()
	{
		...
		NiteWrapper.SetSkeletonSmoothing(0.3);		
	}
